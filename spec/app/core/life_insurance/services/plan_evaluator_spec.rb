...
require 'spec_helper'
require_relative '../../../../../app/core/life_insurance/services/plan_evaluator'

RSpec.describe LifeInsurancePlanEvaluator do # rubocop:disable Metrics/BlockLength
  let(:validator_service) { instance_double('Validator') }
  let(:insurance_plan_categorizer) { instance_double('InsurancePlanCategorizer') }
  let(:eligibility_validator) { instance_double('LifeInsuranceEligibilizer') }
  let(:risk_score_calculator) { instance_double('LifeInsuranceRiskScorer') }
  ...
  describe '#call' do # rubocop:disable Metrics/BlockLength
    ...
    context 'when all parameters are valid' do # rubocop:disable Metrics/BlockLength
