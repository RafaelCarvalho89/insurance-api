...
require 'spec_helper'
require_relative '../../../../../app/core/home_insurance/services/plan_evaluator'

RSpec.describe HomeInsurancePlanEvaluator do # rubocop:disable Metrics/BlockLength
  let(:validator_service) { instance_double('Validator') }
  let(:insurance_plan_categorizer) { instance_double('InsurancePlanCategorizer') }
  let(:eligibility_validator) { instance_double('HomeInsuranceEligibilizer') }
  let(:risk_score_calculator) { instance_double('HomeInsuranceRiskScorer') }
